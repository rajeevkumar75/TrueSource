...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train frame-level deepfake detector used for video classification"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default="data/deepfake_images",
        help="Dataset root (supports train/val/test, training/validation/testing, or class-only folders)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="If set (e.g. 2.0), train with focal loss to emphasize hard fakes instead of cross-entropy",
    )
    parser.add_argument(
